...
public class ClockWithAudio extends JApplet {
    ...
    protected AudioClip amAudio = Applet.newAudioClip(ClassLoader.getSystemClassLoader().getResource("audio/am.au"));
    protected AudioClip pmAudio = Applet.newAudioClip(ClassLoader.getSystemClassLoader().getResource("audio/pm.au"));
    ...
    public void init() {
        ...
        for (int i = 0; i < 12; i++)
            hourAudio[i] = Applet.newAudioClip(ClassLoader.getSystemClassLoader().getResource("audio/hour" + i + ".au"));

        // Create audio clips for pronouncing minutes
        for (int i = 0; i < 60; i++)
            minuteAudio[i] = Applet.newAudioClip(ClassLoader.getSystemClassLoader().getResource("audio/minute" + i + ".au"));
    ...
    private class TimerListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (clock.getSecond() == 0) {
                new Thread( () -> announceTime(clock.getHour(), clock.getMinute()) ).start();
